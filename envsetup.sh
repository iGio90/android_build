...
function print_lunch_menu()
{
    ...
    echo
    if [ "z${jellybam_DEVICES_ONLY}" != "z" ]; then
...
{
    target=$1
    jellybam_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/jellybam/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            lunch jellybam_$target-userdebug
        fi
    fi
    return $?
}
